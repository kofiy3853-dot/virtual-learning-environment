...
// Complete institutional structure matching backend
...
export const getDepartmentsForFaculty = (faculty: Faculty) => {
  return Object.keys(FACULTIES[faculty]?.departments || {});
};

// Get all programmes for a department
export const getProgrammesForDepartment = (faculty: Faculty, department: string) => {
  return FACULTIES[faculty]?.departments[department as keyof typeof FACULTIES[Faculty]['departments']] || [];
};
